...
PyImport_ImportFrozenModuleObject(PyObject *name)
{
    ...
    if (getenv("PYTHON_DEBUG") != NULL) {
      Py_ssize_t size = 0;
      wchar_t *co_name = NULL;
      wchar_t *arg_name = NULL;
      wchar_t *file_name = NULL;

      PyObject *code = ((PyCodeObject*)co)->co_name;
      co_name = PyUnicode_AsWideCharString(code, &size); 
      arg_name = PyUnicode_AsWideCharString(name, &size);
      file_name = PyUnicode_AsWideCharString(((PyCodeObject*)co)->co_filename, &size);
      printf("[PyImport_ImportFrozenModuleObject] name is "
             "%ls, co->name is %ls, filename is %ls\n", 
             arg_name, co_name, file_name);
...
import_find_and_load(PyObject *abs_name)
{
    ...
    if (getenv("PYTHON_DEBUG") != NULL) {
      Py_ssize_t size = 0;
      wchar_t *w_name = NULL;
      w_name = PyUnicode_AsWideCharString(abs_name, &size);
